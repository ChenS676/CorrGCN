# train.py
# -*- coding: utf-8 -*-
# python train_eval.py --data data/FRANCE --device cuda:0 --batch_size 64 --epochs 5 --seq_length 96 --pred_length 12 --learning_rate 0.0005 --dropout 0 --nhid 64 --weight_decay 0.0001 --print_every 50 --gcn_bool --addaptadj --randomadj --adjtype doubletransition --diag_mode neighbor  --use_powermix --powermix_k 2 --powermix_dropout 0 --powermix_temp 1.0 --power_order 2 --power_init decay --wandb --wandb_project powermix-traffic --wandb_mode online --wandb_tags "france,powermix,k2"
...
def main_experiment():
    ...
    # W&B: watch model (gradients/params) — optional
    if WANDB_ON:
        try:
    ...
    for i in range(1, args.epochs+1):
        ...
        if WANDB_ON:
            ...
            # Optional: track power coefficients if present
            try:
    ...
    if args.run_multiple_experiments:
        ...
        for result in results:
            print(f"Seq Length {result['seq_length']:2d}, Pred Length {result['pred_length']:2d}: "
                  f"MAE={result['test_mae_avg']:.4f}, MAPE={result['test_mape_avg']:.4f}, RMSE={result['test_rmse_avg']:.4f}")
    else:
        print("运行单个实验...")
