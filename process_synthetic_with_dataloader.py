...
def _make_windows(
    ...
    默认 C_in=2：第1维是数值本身；第2维给一个简单的时间编码（归一化的 time-of-day 占位）
    """
    N, T = data_arr.shape
    max_start = T - (seq_len + pred_len)
    xs, ys = [], []
    # 简易时间编码（0~1）
    tcode = (np.arange(T) % 288) / 288.0  # 若非交通数据：只是提供第二通道以配合 in_dim=2 的模型
...
# python /mnt/webscistorage/cc7738/ws_joella/EnergyTSF/GNN/Graph-WaveNet-master-origin/process_synthetic_with_dataloader.py 
